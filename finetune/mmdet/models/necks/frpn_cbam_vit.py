...
class FRPN_CBAM_VIT(BaseModule):
    ...
    def __init__(self,
        ...
        super(FRPN_CBAM_VIT, self).__init__(init_cfg)
    ...
    def forward(self, inputs):
        ...
        '''First, the average-pooling is adopted to reduce the sizes of [P2, P3, P4], and obtain the intermediate feature maps'''
        # F.interpolate(laterals[i], size=prev_shape, **self.upsample_cfg)
        # F.max_pool2d(outs[-1], 1, stride=2)
        im_FP = []
        im_FP.append(F.avg_pool2d(outs[0], 2, stride=2))
        im_FP.append(F.avg_pool2d(outs[1], 2, stride=2))
        im_FP.append(F.avg_pool2d(outs[2], 2, stride=2))
        im_FP.append(F.avg_pool2d(outs[3], 2, stride=2))

        frpn = [outs[0]]
        add = [outs[i+1] + im_FP[i] for i in range(len(im_FP))]
        frpn.extend(add)

        # '''计算weights'''
        # """记得卷积加激活函数（sigmoid，ReLU不行）限制weights范围，不然exp(weights)中容易出现Inf，从而lamda系数出现NAN值""" 
        # conv1 = ConvModule(256, 64, 1, inplace=False).to("cuda")
        # # conv1 = ConvModule(256, 64, 1, conv_cfg=None, norm_cfg=None, act_cfg=None, inplace=False).to("cuda")
        # conv3 = ConvModule(64, 1, 3, padding=1, act_cfg=dict(type='Sigmoid'), inplace=False).to("cuda")
        ...
        # '''求FL'''
        # l = [torch.FloatTensor(4, 1, outs[2].shape[2], outs[2].shape[3]).to("cuda") for i in range(5)]
        # for i in range(5):
        ...
        # outs1.append(l_sum)
        # outs1.append(F.max_pool2d(l_sum, 1, stride=2))
        # outs1.append(F.max_pool2d(l_sum, 1, stride=4))
