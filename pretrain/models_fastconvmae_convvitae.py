...
from vision_transformer import PatchEmbed, Block, CBlock, PCT_Block
...
class MaskedAutoencoderViTAE(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        #     for i in range(depth[2])])
        self.blocks3 = nn.ModuleList([
            NormalCell(dim=embed_dim[2], num_heads=num_heads, mlp_ratio=mlp_ratio[2],  qkv_bias=True, qk_scale=None, norm_layer=norm_layer, 
                       kernel=1, class_token=False, group=embed_dim[2] // 4, mlp_hidden_dim=None)
            for i in range(depth[2])])
...
def fastconvmae_convvitae_base_patch16_dec512d8b(**kwargs):
...
fastconvmae_convvitae_base_patch16 = fastconvmae_convvitae_base_patch16_dec512d8b  # decoder: 512 dim, 8 blocks